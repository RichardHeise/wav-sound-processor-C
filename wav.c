...
void readAudioData(wav_t *wav_pointer, FILE *input) {
    ...
    wav_pointer->audio = malloc (sizeof (int16_t) * wav_pointer->header.data.SubChunk2Size /
                                 sizeof(int16_t));
...
void writeAudioData(wav_t *wav_pointer, FILE *output) {
    ...
    fwrite(wav_pointer->audio, sizeof (int16_t), wav_pointer->header.data.SubChunk2Size / 
            sizeof(int16_t), output);
...
void normalizer(wav_t *wav_pointer) {
    ...
    peak = max (wav_pointer->audio, wav_pointer->header.data.SubChunk2Size /
                sizeof(int16_t));
    diff = (MAX_VOL - peak);
...
void mixWavs (wav_t *wavs, int size, wav_t *wav_out) {
    ...
    for (i = 0; i < size; i++) {
        mixAudio (wavs[i].audio, wavs[i].header.data.SubChunk2Size / 
                    sizeof(int16_t),wav_out->audio);
        free(wavs[i].audio);
    }
}
...
void concatWavs(wav_t *wavs, int size, wav_t *wav_out) {
    ...
    for (i = 0; i < size; i++) {
        ...
        copyAudio(wavs[i].audio, wavs[i].header.data.SubChunk2Size / 
                    sizeof(int16_t), wav_out->audio, &currentPos);
